<template>
  <div class="otf-table-wrap">
    <DataTable id="tabla" ref="dt" class="fm-pass-grid otf-table" :value="store.rows" dataKey="id" tableStyle="table-layout: fixed; width: max-content; min-width: 100%" scrollable scrollHeight="430px" :rowClass="rowClass" :resizableColumns="true" columnResizeMode="expand" removableSort sortMode="multiple" filterDisplay="row" v-model:filters="filters" v-model:selection="selectedRows" selectionMode="multiple" :isDataSelectable="isRowSelectable" :selectAll="allSelectableSelected" paginator :rows="10" :rowsPerPageOptions="[10, 50, 100, 500]" paginatorTemplate="FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink RowsPerPageDropdown" currentPageReportTemplate="Pagina {currentPage} de {totalPages}" showGridlines @row-click="onRowClick" @select-all-change="onSelectAllChange">
      ...
      <template #paginatorend><span class="otf-counter">Mostrando {{ store.rows.length ? 1 : 0 }} - {{ Math.min(10, store.rows.length) }} de {{ store.rows.length }}</span></template>
      <template #empty><div class="otf-empty">No hay resultados</div></template>
      ...
      <Column v-for="col in cols" :key="col.field" :field="col.field" :header="col.header" :sortable="col.sort !== false" :filter="col.filter !== false" :filterField="col.field" :showFilterMenu="false" :hidden="col.hidden" :exportable="col.exportable" :style="{ minWidth: col.width || '120px' }">
        <template #filter="{ filterModel, filterCallback }"><InputText v-if="col.filter !== false" type="text" v-model="filterModel.value" @input="filterCallback()" class="fm-column-filter" /></template>
        <template #body="{ data }"><button v-if="col.field === 'tieneNota' && data.nota" class="otf-icon-cell" type="button" @click.stop="verNota(data)"><span class="material-icons">edit_note</span></button><button v-else-if="col.field === 'incluir' && data.excluida === 'S'" class="otf-icon-cell otf-icon-cell--include" type="button" @click.stop="abrirIncluir(data)"><span class="material-icons">settings_backup_restore</span></button><span v-else-if="col.field !== 'tieneNota' && col.field !== 'incluir'">{{ data[col.field] ?? '' }}</span></template>
      </Column>
    </DataTable>
...
const onSelectAllChange = (event) => { store.setSelectedRows(event.checked ? selectableRows.value.map((row) => row.id) : []) }
...
<style scoped>
.otf-table-wrap{border-left:4px solid var(--fm-cyan);background:#fff}.otf-footer-icons{display:flex;align-items:center;gap:10px;padding-left:4px}.otf-counter{font-size:12px;color:#222;padding-right:8px}.otf-empty{padding:48px 32px;font-size:14px;color:#407080;text-align:center}.otf-icon-cell{border:0;background:transparent;color:var(--fm-cyan);cursor:pointer;padding:0;font-size:15px;display:inline-flex;align-items:center;justify-content:center}.otf-icon-cell .material-icons{font-size:18px}.otf-icon-cell:hover{color:var(--fm-cyan-strong);transform:translateY(-1px)}.otf-icon-cell--include{color:var(--fm-cyan)}.fm-icon-btn .material-icons{font-size:18px}:deep(.p-datatable-tbody > tr){cursor:pointer}:deep(.otf-disabled-row td){color:#8b8b8b!important;background:#fff!important}:deep(.otf-enabled-row:hover td),:deep(.otf-disabled-row:hover td){background:#edfafd!important}:deep(.p-datatable-tbody > tr.p-highlight > td),:deep(.p-datatable-tbody > tr.p-datatable-row-selected > td){background:var(--fm-cyan-row)!important;color:#003f47!important}:deep(.p-datatable-thead > tr > th){height:36px!important;padding:4px 7px!important}:deep(.p-datatable-tbody > tr > td){height:36px!important;padding:4px 7px!important}:deep(.p-filter-row > th){height:34px!important;background:#fff!important}:deep(.p-column-filter){height:25px!important;font-size:12px!important}:deep(.p-datatable-wrapper){min-height:186px;max-height:470px}:deep(.p-datatable-empty-message td){height:110px!important;background:#eafcff!important;text-align:center!important}:deep(.p-paginator){justify-content:center;position:relative}:deep(.p-paginator-left-content){position:absolute;left:8px}:deep(.p-paginator-right-content){position:absolute;right:8px}
</style>